...
void z21Class::receive(uint8_t client, uint8_t *packet) 
{
	...
	switch (header) {
		...
		case (0x13): {	//configuration write
			...
			/* Daten:
			50: (0x01) RailCom: 0=aus/off, 1=ein/on
			51: (0x00)
			52: (0x01) Power-Button: 0=Gleisspannung aus, 1=Nothalt
			53: (0x03) Auslese-Modus: 0=Nichts, 1=Bit, 2=Byte, 3=Beides
			54: (0x01) 
			55: (0x00) 
			56: (0x03) 
		...
		case (0x17): {	//configuration write
			...
			/* Daten:
			60-4: (0x19) Reset Packet (starten) (25-255)
			61-5: (0x06) Reset Packet (fortsetzen) (6-64)
			62-6: (0x07) Programmier-Packete (7-64)
			63-7: (0x01) ?
			64-8: (0x05) ?
			65-9: (0x14) ?
			66-10: (0x88) ?
			...
			69-13: (0x27) ?
			70-14: (0x32) ?
			71-15: (0x00)  0x03 -> Ausgabeformat (0x00) DCC+MM, (0x02) nur DCC, (0x03) nur MM
						0x40 -> DCC Funktion ab F13 periodisch wiederholen
						0x80 -> kurze DCC Lokaddressen (0x00 = 1..99 / 0x80 = 1..127)
			72: (0x50) Hauptgleis (LSB) (11-23V)
			73: (0x46) Hauptgleis (MSB)
			74: (0x20) Programmiergleis (LSB) (11-23V): 20V=0x4e20, 21V=0x5208, 22V=0x55F0
			75: (0x4e) Programmiergleis (MSB)
			*/
			#if defined(SERIALDEBUG)
				...
				switch (packet[15] & 0x03) {
					case 0x00: ZDebug.print("DCC+MM"); break;	//Ausgabeformat DCC+MM
					case 0x01: ZDebug.print("error"); break;	
					case 0x02: ZDebug.print("DCC"); break;		//Ausgabeformat nur DCC
					case 0x03: ZDebug.print("MM"); break;		//Ausgabeformat nur MM
				}
